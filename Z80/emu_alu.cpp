...
EmuAlu::EmuAlu(Memory *memory, IO *io, Registers *registers) : memory(memory), io(io), registers(registers) {
...
void EmuAlu::ADD(RegisterPair destination, RegisterPair rgstr) {
  ...
  registers->setRegisterPair(destination,
                             registers->getRegisterPairValue(destination) + registers->getRegisterPairValue(rgstr));
}
...
void EmuAlu::ADD(Rgstr destination, Rgstr source) { ADD(destination, registers->getRegisterValue(source)); }

void EmuAlu::ADD(Rgstr destination, MemoryAddress memoryAddress) {
  ADD(destination, memory->read(getMemoryAddress(memoryAddress)));
}

void EmuAlu::AND(Rgstr iX2) { AND(registers->getRegisterValue(iX2)); }
...
void EmuAlu::AND(MemoryAddress memoryAddress) { AND(memory->read(getMemoryAddress(memoryAddress))); }

/**
 * This instruction tests bit b in rgstr r and sets the Z flag
 * accordingly.
 */
void EmuAlu::BIT(std::uint8_t y, std::uint8_t value) { registers->setZeroFlag(!((value & (1 << y)) == (1 << y))); }

void EmuAlu::BIT(std::uint8_t y, Rgstr rgstr) { BIT(y, registers->getRegisterValue(rgstr)); }

void EmuAlu::BIT(std::uint8_t i, MemoryAddress memoryAddress) { BIT(i, memory->read(getMemoryAddress(memoryAddress))); }
...
void EmuAlu::CP(Rgstr val) { CP(registers->getRegisterValue(val)); }

void EmuAlu::CP(MemoryAddress memoryAddress) { CP(memory->read(getMemoryAddress(memoryAddress))); }
...
void EmuAlu::HALT() { registers->setHalted(true); }

void EmuAlu::IM(std::uint8_t im) { registers->setIM(im); }
 ...
 * of the PC.
 */
void EmuAlu::JP(MemoryAddress memoryAddress) { registers->setPC(getMemoryAddress(memoryAddress)); }
...
void EmuAlu::JR(MemoryAddress memoryAddress) { registers->setPC(getMemoryAddress(memoryAddress)); }
...
void EmuAlu::LD(RegisterPair rgstrPair, MemoryAddress memoryAddress) {
  std::uint16_t value =
      memory->read(getMemoryAddress(memoryAddress)) | (memory->read(getMemoryAddress(memoryAddress) + 1) << 8);
  registers->setRegisterPair(rgstrPair, value);
}

void EmuAlu::LD(MemoryAddress memoryAddress, std::uint8_t i) { memory->write(getMemoryAddress(memoryAddress), i); }

void EmuAlu::LD(Rgstr rgstr, std::uint8_t immediateValue) { registers->setRegister(rgstr, immediateValue); }
...
void EmuAlu::OR(Rgstr rgstr) { OR(registers->getRegisterValue(rgstr)); }
...
void EmuAlu::OR(MemoryAddress memoryAddress) { OR(memory->read(getMemoryAddress(memoryAddress))); }
...
void EmuAlu::RETI() { unimplemented("RETI"); }

/* Return from non maskable interrupt */
void EmuAlu::RETN() { unimplemented("RETN"); }
...
copied to bit 0.
*/
void EmuAlu::RL(MemoryAddress memoryAddress) { unimplemented("RL"); }
 ...
 * copied to bit 0.
 */
void EmuAlu::RL(Rgstr r) { unimplemented("RL"); }
...
void EmuAlu::RLC(MemoryAddress memoryAddress) { unimplemented("RLC"); }

void EmuAlu::RLC(Rgstr rgstr) { unimplemented("RLC"); }
 ...
 */
void EmuAlu::RLCA() {
  registers->setA(((registers->getA() << 1) & BOOST_BINARY(11111111)) |
                  ((registers->getA() >> (8 - 1)) & BOOST_BINARY(00000001)));
  registers->setCFlag((registers->getA() & BOOST_BINARY(1)) == 1);
}

void EmuAlu::RLD() { unimplemented("RLD"); }

void EmuAlu::RR(MemoryAddress memoryAddress) { unimplemented("RR"); }

void EmuAlu::RR(Rgstr r) { unimplemented("RR"); }
...
void EmuAlu::RRC(MemoryAddress memoryAddress) { unimplemented("RRC"); }

void EmuAlu::RRC(Rgstr r) { unimplemented("RRC"); }
...
void EmuAlu::RRD() {
  ...
  memory->write(registers->getHL(), (priorMemory >> 4) | ((priorAccumulator & BOOST_BINARY(1111)) << 4));
}
...
void EmuAlu::SBC(Rgstr a, MemoryAddress memoryAddress) { SBC(a, memory->read(getMemoryAddress(memoryAddress))); }
...
void EmuAlu::SBC(Rgstr a, Rgstr b) { SBC(a, registers->getRegisterValue(b)); }
...
void EmuAlu::SLA(MemoryAddress memoryAddress) { unimplemented("SLA"); }

void EmuAlu::SLA(Rgstr r) { unimplemented("SLA"); }

void EmuAlu::SRA(MemoryAddress memoryAddress) { unimplemented("SRA"); }

void EmuAlu::SRA(Rgstr r) { unimplemented("SRA"); }

void EmuAlu::SRL(MemoryAddress memoryAddress) { unimplemented("SRL"); }
...
C is set if borrow; reset otherwise
*/
void EmuAlu::SUB(Rgstr reg) { SUB(registers->getRegisterValue(reg)); }
...
void EmuAlu::SUB(MemoryAddress memoryAddress) { SUB(memory->read(getMemoryAddress(memoryAddress))); }

void EmuAlu::XOR(Rgstr val) { XOR(registers->getRegisterValue(val)); }
...
void EmuAlu::XOR(std::uint8_t val) {
  ...
  std::uint8_t testVal = val;
  for (std::uint8_t i = 0; i < 8; ++i) {
    parity ^= testVal & 1;
    testVal >>= 1;
    std::cout << parity << std::endl;
  }
  registers->setParityOverflowFlag(parity);
}

void EmuAlu::XOR(MemoryAddress memoryAddress) { XOR(memory->read(getMemoryAddress(memoryAddress))); }

void EmuAlu::decrementSP() { registers->setSP(registers->getSP() - 1); }

void EmuAlu::incrementSP() { registers->setSP(registers->getSP() + 1); }
...
std::uint8_t EmuAlu::readIO(std::uint16_t address) { return io->read(address); }
